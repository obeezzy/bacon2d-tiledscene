...
public:
    ...
    Q_INVOKABLE QString getProperty(const QString &name) const;
    ...
    int count() const { return m_collisionItems.count(); }
    Q_INVOKABLE void reset() { setCollisionIndex(-1); }
...
class CollisionItem : public QQuickItem
{
    Q_OBJECT
public:
    CollisionItem(QQuickItem *parent = 0) : m_body(0), m_collisionIndex(-1), QQuickItem(parent) {}
